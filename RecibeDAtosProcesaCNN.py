...
def analizar_mosquito(file_path, model):
    ...
    # --- CÁLCULO DE FRECUENCIA CENTRAL CORREGIDO ---
    S = np.abs(librosa.stft(y))
    f = librosa.fft_frequencies(sr=sr)
    # Filtramos el espectro para que solo vea el rango del mosquito (200-1500Hz)
    idx = np.where((f >= 200) & (f <= 1500))[0]
    if len(idx) > 0:
        centroids = librosa.feature.spectral_centroid(S=S[idx, :], sr=sr)
        freq_central = np.mean(centroids)
    else:
        freq_central = 0.0
    ...
    # Obtenemos el valor escalar de la probabilidad
    ...
    return probabilidad, freq_central, latencia
